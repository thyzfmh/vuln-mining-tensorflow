...
for F in submission/vulnerability_list.md submission/llm_chat_log.json submission/vulnerability_report.md submission/verify/run_test.py; do
  if [ ! -f "$F" ]; then
...
if [ -f "submission/llm_chat_log.json" ]; then
  if python3 -c "import json; d=json.load(open('submission/llm_chat_log.json')); assert 'metadata' in d; assert 'chat_history' in d; assert len(d['chat_history']) >= 1" 2>/dev/null; then
    TURN_COUNT="$(python3 -c "import json; d=json.load(open('submission/llm_chat_log.json')); print(len(d.get('chat_history', [])))")"
...
if [ -f "submission/verify/run_test.py" ]; then
  ...
  if grep -q 'def run_test\|def test_case' submission/verify/run_test.py 2>/dev/null; then
...
if [ -f "submission/vulnerability_list.md" ]; then
  if grep -q '问题源码路径\|source.*path\|源码路径' submission/vulnerability_list.md; then
  ...
  if grep -q '验证结果\|verification\|CRASH\|crash' submission/vulnerability_list.md; then
...
if [ -f "submission/llm_chat_log.json" ]; then
  ...
  if grep -qiE '"这个项目是.*[Tt]ensor[Ff]low"|这是.*版本|这个版本有.*bug' submission/llm_chat_log.json 2>/dev/null; then
...
if [ -f "submission/vulnerability_report.md" ]; then
  if grep -qi 'code semantic analysis\|No CVE\|AI generated\|runtime execution' submission/vulnerability_report.md; then
...
if [ "$ERRORS" -eq 0 ]; then
...

  {
    ...
    echo "## Deliverables"
    echo ""
    VULNS="$(grep -c '^## 漏洞 #' submission/vulnerability_list.md 2>/dev/null || echo '0')"
    TURNS="$(python3 -c "import json; d=json.load(open('submission/llm_chat_log.json')); print(len(d.get('chat_history', [])))" 2>/dev/null || echo '0')"
