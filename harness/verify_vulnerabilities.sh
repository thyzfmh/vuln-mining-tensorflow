...
if [ -f "vulnerability_list.md" ]; then
  ...
  for SECTION in "漏洞类型" "严重级别" "问题源码路径" "成因简述" "验证结果"; do
    if ! grep -q "$SECTION" vulnerability_list.md; then
      echo "WARNING: Missing section '$SECTION' in vulnerability_list.md" >&2
      WARNINGS=$((WARNINGS + 1))
    fi
  done
else
  echo "ERROR: vulnerability_list.md not found" >&2
  ERRORS=$((ERRORS + 1))
fi

if [ -f "llm_chat_log.json" ]; then
  if ! python3 -c "import json; json.load(open('llm_chat_log.json'))" 2>/dev/null; then
  ...
  echo "ERROR: llm_chat_log.json not found" >&2
  ERRORS=$((ERRORS + 1))
fi
...
if [ -f "vulnerability_report.md" ]; then
  ...
  echo "ERROR: vulnerability_report.md not found" >&2
  ERRORS=$((ERRORS + 1))
fi
...
if [ -f "verify/run_test.py" ]; then
  ...
  echo "ERROR: verify/run_test.py not found" >&2
  ERRORS=$((ERRORS + 1))
fi
